"""ModernBERT model for MLX (vendored from mlx_embeddings to avoid heavy import chain)."""
...
class ModelArgs(BaseModelArgs):
    ...
    is_regression: bool | None = None  # for Sequence Classification
    label2id: dict[str, int] | None = None  # for Sequence Classification
    id2label: dict[int, str] | None = None  # for Sequence Classification
    pipeline_config: dict[str, Any] | None = None  # for Sequence Classification
...
class ModernBertAttention(nn.Module):
    ...
    def __init__(self, config: ModelArgs, layer_id: int | None = None):
        super().__init__()
        self.config = config
...
class ModernBertModel(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.layers = [ModernBertEncoderLayer(config, i) for i in range(config.num_hidden_layers)]
...
class Model(nn.Module):
    ...
    def __call__(
        ...
        last_hidden_state = (
            encoder_outputs["last_hidden_state"] if isinstance(encoder_outputs, dict) else encoder_outputs[0]
        )
