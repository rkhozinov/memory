...
- **Progressive disclosure**: `memory search` / `memory doc search` take `--depth {summary,full}`. `full` is the default and dumps the complete JSON record; `summary` prints `<hash16> [<type>] score=<n.nn> <preview>` (docs: `<hash16> [doc] <title>`), with a trailing `[stale:N]` when the hit carries `stale_refs`. `search` also takes `--min-score` to drop low-scoring hits (unscored modes like `exact` pass through). Note hybrid fusion scores are not capped at 1.0. `memory admin clusters` has its own `--depth {summary,full}`, defaulting to `summary`. `memory doc list` also takes `--depth {summary,full}` (default `full`); `summary` returns the same JSON records with `body` omitted — a full listing of 2 session archives is 142 KB against 2.5 KB, almost entirely bodies. The MCP server uses a separate `depth: titles` projection.
- **Markdown mirror**: `export_markdown(dest)` writes one file per record — memories to `<dest>/memories/<type>/<hash16>.md`, documents to `<dest>/documents/<slug>-<hash16>.md` — with YAML frontmatter and the body below it. Names come from the content hash, so a re-run overwrites in place. It does **not** prune: a record deleted since the last run leaves its file behind, so delete the directory and re-export if that matters. Filename clashes are counted and returned as `collisions` rather than silently overwritten — the `documents` table has no unique constraint on `content_hash`, so two rows really can map to one filename.
